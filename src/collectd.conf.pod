...
Enables or disables the plugin-specific alignment of calls to read functions. 
This overrides the global B<AlignRead> setting. 

=item B<AlignReadOffset> I<Seconds>

Enables aligned calls to read functions of the plugin and additionally specifies
an offset, in seconds, until the interval begin will be further delayed.
...
If enabled, the call to read functions is set to a multiple of the read 
